function b=revmakeregressor115251(id,filenum)
...
%     id5=sprintf('%d',id);
% end
subdir=sprintf('115251');

cd(subdir)
id5=15251;
fname=sprintf('alexRevlearnHiLo3-%d-%d.txt',id5,filenum);
...
%% see if the correct stimulus was on the left or right [OR VICE
%VERSA???????]
...
b.feed2=[];
for ct=1:blocklength
    %showstim.ACC: 0=error, 1=correct; maybe wrong, run with this for the
    %min
   if b.choice(ct)<3
...
%     goodtrialslist=find(goodtrialsbeforereversal);
%     %not sure what to do with poorly performing subjects here
%     if length(goodtrialslist)<5
%         lastfivegoodtrials=lastfivegoodtrials;
%     else
%     lastfivegoodtrials=[lastfivegoodtrials; goodtrialslist(end-4:end)];
%     end
% end
...
% learn=8;
% for ct=1:length(reversals-1)
%     %first see where they reached learning criterion
%     learnedafterreversal=find(b.NumRight==learn & trialnum>reversals(ct) & trialnum<(reversals(ct)+25), 1);
%     if isempty(learnedafterreversal)==1
%         b.pass(ct)=0;
%     else
...
% %make regressors for choice and feedback for modeling
% 
% %first see if the correct stimulus was on the left or right [OR VICE
% %VERSA???????]
% for ct=1:length(b.TrialType)
% b.corrleft(ct)=strcmp(b.TrialType(ct), ' BA');
% end
% 
% %This is written for block that have been aborted or ran too long (v.3)
% if 100<=length(b.Running) && length(b.Running)<110
%     blocklength=100;
% else
%     blocklength=length(b.Running);
% end
...
% b.responsetrials=find(b.choice<3);
% b.goodchoice=find(b.choice(b.responsetrials));
% %find and count probabilistic switches
% b.switch=[];
% b.switchnum=0;
% for ct=2:length(b.choice)
%        if b.choice(ct)~=b.choice(ct-1) && b.choice(ct)<3
...
%        end
% end
% %find and count perseverative errors
% b.persev=[];
% b.persevnum=0;
% b.persev=((b.switch'==0)&(b.NumRight(1:blocklength)==0))';
% b.persevnum=sum(b.persev);
