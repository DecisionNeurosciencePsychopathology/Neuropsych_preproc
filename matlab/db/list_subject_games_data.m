...
function list_subject_games_data( varargin )
...
fprintf(fid,'\\begin{longtable}{ccccccp{10mm}c}\n\n');
...
fprintf(fid,'\t\t\\multicolumn{2}{c}{N = %d} \t & N = %d \t & N = %d \t & N = %d \t & N = %d \\\\ \\hline\n', ...
    numel(ids),sum(data.bandit),sum(data.bart),sum(data.ultimatum),sum(data.willtowait));
...
fprintf(fid,'\\textbf{BART}\t&\t');
fprintf(fid,'\\textbf{Ultimatum Game}\t&\t');
fprintf(fid,'\\textbf{Willingness to Wait} \\\\ \\hline\n');
fprintf(fid,'\t\t\\endhead\n\n');
...
for ni = 1:numel(data.id)
    ...
    % write line
    fprintf(fid,'%s\t & \t%s\t & \t%s\t & \t%s \\\\ \\hdashline[1pt/2pt]\n',s.bandit,s.bart,s.ultim,s.willin);
